...
void diag_line(unsigned short x1, unsigned short y1, unsigned short ix, short incx, short incy)
{
    ...
    */
    for( step = 0; (wall == FALSE) && (step < 6); ++step )
    {
        ...
        */
        ...
        {
            line( sszx, sszy, sszxp1, sszyp1 );
            line( sszx, LABYRINTHSZY_DYN - sszy, sszxp1, LABYRINTHSZY_DYN - sszyp1 );
        }
        else
        {
            // Closer vertical line
            line( sszx, sszy, sszx, LABYRINTHSZY_DYN - sszy );
            // Farter vertical line
            if( labyrinth[posx + advancex + (posy + advancey) * labyrinthSizeX] != '*' )
            {
                line( sszxp1, sszyp1, sszxp1, LABYRINTHSZY_DYN - sszyp1 );
            }
            // Upper horisontal line
            line( sszxp1, sszyp1, sszx, sszyp1 );
            // Lower horisontal line
            line( sszxp1, LABYRINTHSZY_DYN - sszyp1, sszx, LABYRINTHSZY_DYN - sszyp1 );
        }
    ...
    if( wayout )
    {
        ...
        LABYRINTHSZX_DYN - step * LABSTEPSZX_DYN, LABYRINTHSZY_DYN - step * LABSTEPSZY_DYN );
        line( LABYRINTHSZX_DYN - step * LABSTEPSZX_DYN, step * LABSTEPSZY_DYN, step * LABSTEPSZX_DYN,
    ...
    while( TRUE )
    {
        if( oldx != positionx || oldy != positiony || oldo != orientation )
        {
            ...
            if( positionx == exitx && positiony == exity )
            {
                ...
                oldx = 0;
                goto restart;
                // No program for the C64 would be complete
